...
## Usage: sed 's/transcript/gene/g' input.gff3 | sed 's/gene_name/Name/g' | sed 's/geneID/Alias/g' | pre_gff3_for_misopy_sashimi.awk > output.sashimi.gff3
{
	if ($3 == "gene"){
		exon_counter=0
		...
		gene_attributes=sprintf("ID=%s;Name=%s;Alias=%s",temp_Name,temp_Name,temp_Alias);
		mRNA_attributes=sprintf("ID=%s;Parent=%s",temp_ID,temp_Name)
