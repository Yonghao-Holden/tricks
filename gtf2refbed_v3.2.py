...
# run it as followed:
# /bar/yliang/tricks/gtf2refbed_v3.py --input <input.gtf>
'''
import argparse
...
def main():
	...
	if input_type == "stringtie" or input_type == "gencode":
	...
	subprocess.run("rm "+input_gtf_file.replace("gtf", "refbed"), shell=True)
		...
		for line in input_file:
			...
			if entry[2] in useful_info:
				...
				if entry[2] == "transcript":
					input_data[transcript_id] = [[entry[0], str(int(entry[3])-1), entry[4], entry[3], entry[4], entry[6], gene_name, transcript_id, transcript_type],[],[],[detail_info]]
				elif entry[2] == "exon":
					input_data[transcript_id][1].append(str(int(entry[3])-1))
					input_data[transcript_id][2].append(entry[4])
				elif entry[2] == "start_codon":
					input_data[transcript_id][0][3] = str(int(entry[3])-1)
...
def gtf_to_refbed_taco(input_gtf_file):
	""" convert stringtie gtf file to refbed file for washu genome browser visualization """
	# chr, transcript_start, transcript_stop, translation_start, translation_stop, 
	# strand, gene_name, transcript_id, type, exon(including UTR bases) starts, 
	# exon(including UTR bases) stops, and additional gene info (optional)
