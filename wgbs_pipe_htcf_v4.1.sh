...
dependtrimming="afterok:$jid"
	...
	date
	find ${trimdir} -maxdepth 1 -name "*.gz"  | while read file ; do xbase=\\\$(basename \\\$file) ; mkdir ${trimdir}/\\\${xbase%.*}_fastqc ; echo "/home/yonghao/softwares/anaconda3/bin/fastqc -o "${trimdir}/\\\${xbase%.*}"_fastqc "\\\$file"" >> ${trimdir}/1b_fastqc_commands.txt; done ;
