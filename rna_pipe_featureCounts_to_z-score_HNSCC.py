...
def origin(cell_line):
	if "SCC90" in cell_line or "SCC-90" in cell_line:
		return("Oropharynx")
	elif "SCC-47" in cell_line or "SCC47" in cell_line:
		return("Oral Cavity")
	elif "93VU147T" in cell_line:
		return("Oral Cavity")
	elif "JHU006" in cell_line or "JHU-006" in cell_line:
		return("Oral Cavity")
	elif "JHU029" in cell_line or "JHU-029" in cell_line:
		return("Laryngeal")
	elif "SCC9" in cell_line or "SCC-9" in cell_line:
		return("Oral Cavity")
	elif "SCC-38" in cell_line or "SCC38" in cell_line:
		return("Oropharynx")
	else:
		return("Normal")
...
catogory = ["HPV" if ("HPV" in index) else "NEG" for index in pca_df.index]
pca_df['catogory'] = catogory
pca_df['cell_line'] = pca_df.index
pca_df['origin'] = pca_df['cell_line'].apply(lambda x: origin(x))
 
print(pca_df)

color = {"NEG":"Green", "HPV":"Red"}

sns.scatterplot(pca_df.loc[:, dimension[0]], pca_df.loc[:, dimension[1]], hue = pca_df.catogory, palette = color, alpha = 0.6, style= pca_df.origin)
